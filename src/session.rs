...
impl ChatSession {
    ...
    ) -> std::pin::Pin<Box<dyn Stream<Item = Result<NewBingResponseMessage>>>> {
    ...
    pub async fn ask(&mut self, text: &str) -> Result<NewBingResponseMessage> {
        let mut stream = self.ask_stream(text).await?;
    ...
    pub async fn ask_stream(
        &mut self,
        text: &str,
    ) -> Result<Pin<Box<dyn Stream<Item = Result<NewBingResponseMessage>>>>> {
        ...
        *(request.headers_mut()) = headers(&self.uuid, &self.ip);
        ...
        Ok(self.stream(ws_stream).await)
    }
